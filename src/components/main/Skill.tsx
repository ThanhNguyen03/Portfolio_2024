...
import { ANOTHER_SKILL, BE_SKILL_DATA, FE_SKILL_DATA, LANGUAGES_SKILL } from '@/constants'
import { SkillDataProvider } from '../ui/SkillDataProvider'
import { SkillKeyboard } from '../ui/SkillKeyBoard'

const Skill = () => {
  return (
    <section id='skills' className='relative overflow-hidden px-2 py-10 md:px-6 md:py-20 lg:px-10'>
        ...
        <div className='center size-full flex-col gap-6 md:hidden'>
          <div className='z-50 flex flex-wrap items-center justify-around gap-6'>
            {LANGUAGES_SKILL.map((skill, index) => (
              <SkillDataProvider name={skill.name} key={skill.name} icon={skill.icon} index={index} />
            ))}
          </div>
          <div className='z-50 flex flex-wrap items-center justify-around gap-6'>
