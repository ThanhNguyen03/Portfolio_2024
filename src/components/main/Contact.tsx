...
const Contact = () => {
  return (
    <section id='contact' className='relative z-30 px-2 py-16 md:px-6 md:py-24 lg:px-10'>
        ...
        <ScrollReveal direction='up' delay={0.1}>
          <div className='rounded-4 flex flex-col items-center gap-8 border border-violet-700/40 bg-violet-950/20 px-6 py-12 text-center backdrop-blur-sm'>
            <p className='text-18 max-w-[500px] text-white/70'>
              Open to full-time roles, freelance projects, and interesting collaborations. Let&apos;s build something
              together.
            </p>
            ...
            >
              ...
              <span className='font-small-caps text-16 font-semibold'>thanhfng.dev@gmail.com</span>
            </a>
              ...
              {SOCIAL_NAVIGATION.map((social) => (
                ...
                >
                  <div className='rounded-2 border border-white/10 bg-white/5 p-2.5 transition duration-300 group-hover:border-white/25 group-hover:bg-white/10'>
                    <Image src={social.image || ''} alt={social.name} width={22} height={22} />
                  </div>
                  <span className='font-small-caps text-12 text-white/40 group-hover:text-white/70'>{social.name}</span>
                </Link>
              ))}
            </div>
          </div>
